...
| Handling of large files | In read-only mode, virtually unlimited without slowdown up to 2³² lines (Uint32), JavaScript TypedArray handling & memory is the limit (so about 2GB is the limit due to JS limitations). Supports streaming as well. | Gives up somewhere between 200-400MB | Handles large files, but begins freaking out | ✔️ |
